...
str_RUC				=   DataTable("e_NumDocumento", "Buscar_Cliente [Buscar_Cliente]")
...
Sub ParametrosAlta()
	...
	Dim Iterator, Count,rs
		...
		For Iterator = 1 To Count-1
			...
			If rs = DataTable ("e_Tipo_Alta", dtLocalSheet) Then
				JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Actualizar Atributos de").JavaList("Motivo:").Select DataTable ("e_Tipo_Alta", dtLocalSheet)
				Exit for
...
Sub DetalleActividadOrden()
	...
	For Iterator = filas-1 To 0 Step -1
		...
		Else 
		 	DataTable("s_Resultado",dtLocalSheet)="Fallido"
			DataTable("s_Detalle",dtLocalSheet)="La orden "&DataTable("s_Nro_Orden",dtLocalSheet)&" no culminó en estado cerrado, falló en la Actividad "&varselec&""
			Reporter.ReportEvent micFail, DataTable("s_Resultado",dtLocalSheet), DataTable("s_Detalle",dtLocalSheet)
